...
export default function AdminPage() {
  ...
  const [userSortBy, setUserSortBy] = useState<"name_asc" | "member_since_asc" | "coins_desc">("name_asc");
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="card-elevated p-6 animate-slide-up bg-white dark:bg-slate-800/50 border-slate-100 dark:border-slate-700/50" style={{ animationDelay: "0.1s" }}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto md:min-w-[540px]">
                ...
                <select
                  value={userSortBy}
                  onChange={(e) => setUserSortBy(e.target.value as "name_asc" | "member_since_asc" | "coins_desc")}
                  className="w-full sm:w-60 px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 focus:border-accent outline-none transition-all text-sm font-bold shadow-sm"
                >
                  <option value="name_asc">{lang === "ar" ? "ترتيب: أبجدي بالاسم" : "Sort: Name (A-Z)"}</option>
                  <option value="member_since_asc">{lang === "ar" ? "ترتيب: أقدم حساب أولاً" : "Sort: First account created"}</option>
                  <option value="coins_desc">{lang === "ar" ? "ترتيب: الأعلى Padel Coins" : "Sort: Highest Padel Coins"}</option>
                </select>
              </div>
            </div>
            ...
            {(() => {
              ...
              const sortedUsers = [...filteredUsers].sort((a, b) => {
                ...
                if (userSortBy === "member_since_asc") {
                  const aDate = new Date(a.memberSince || "").getTime();
                  const bDate = new Date(b.memberSince || "").getTime();
                  const safeADate = Number.isNaN(aDate) ? Number.MAX_SAFE_INTEGER : aDate;
                  const safeBDate = Number.isNaN(bDate) ? Number.MAX_SAFE_INTEGER : bDate;
                  return safeADate - safeBDate;
                }

                const aBookings = accepted.filter(book => book.phone === a.phone || book.name === a.name).length;
                const bBookings = accepted.filter(book => book.phone === b.phone || book.name === b.name).length;
                const aCoins = aBookings - (a.spentCoins || 0);
                const bCoins = bBookings - (b.spentCoins || 0);
